...
namespace StellarRampage.EnemyDrops
{
    internal class Drop
    {
        ...
        public Drop(Vector2 pos, Texture2D asset, Player player, float radius, float scale = 3.5f)
        {
            this.pos = pos;
            this.player = player;
            anchor = pos;
            int height = asset.Height;
            sprite = new AnimatedSprite(asset, height, height, 6, 0.15f, new Rectangle((int)pos.X, (int)pos.Y, height, height), scale);
        ...
        {
            SoundManager.PlaySound("DropPickedUp", 1);
        }
